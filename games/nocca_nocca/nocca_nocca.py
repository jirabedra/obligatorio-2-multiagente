...
class NoccaNocca(AlternatingGame):
    ...
    def _eval_progress(self, agent: AgentID) -> float:
        """
        Average normalised progress of the agent's pieces toward its goal row.
        Uses the board constants (BLACK_START, BLACK_GOAL, etc.) so it adapts
        automatically if the board dimensions change in board.py.
        Returns a value in [0, 1].
        """
        player = self.agent_name_mapping[agent]
        max_dist = (BLACK_GOAL - BLACK_START) if player == BLACK else (WHITE_START - WHITE_GOAL)

        total, count = 0.0, 0
        for r in range(ROWS):
            for c in range(COLS):
                for h in range(MAX_STACK):
                    if self.board.squares[r, c, h] == player:
                        dist = (r - BLACK_START) if player == BLACK else (WHITE_START - r)
                        total += max(0.0, dist / max_dist)
                        count += 1

        return total / count if count > 0 else 0.0

    def _eval_blocked(self, agent: AgentID) -> float:
        """
        Fraction of the agent's pieces that are blocked by an opponent piece
        sitting above them in the same stack.
        Returns a value in [0, 1].
        """
        player   = self.agent_name_mapping[agent]
        opponent = 1 - player
        ...
        for r in range(ROWS):
            for c in range(COLS):
                stack = self.board.squares[r, c, :]
                for h in range(MAX_STACK):
                    if stack[h] == player:
                        total += 1
                        if any(stack[k] == opponent for k in range(h + 1, MAX_STACK)):
                            blocked += 1

        return blocked / total if total > 0 else 0.0

    def _eval_closest_to_goal(self, agent: AgentID) -> float:
        """
        Normalised progress of the single piece closest to the goal row.
        Ignores all other pieces — useful for Minimax since only one piece
        needs to reach the goal to win.
        Returns a value in [0, 1]: 0.0 = still at start row, 1.0 = at goal row.
        """
        player = self.agent_name_mapping[agent]
        max_dist = (BLACK_GOAL - BLACK_START) if player == BLACK else (WHITE_START - WHITE_GOAL)
    ...
    def eval(self, agent: AgentID) -> float:
        ...
        progress = self._eval_progress(agent)       # in [0, 1]  — higher is better
        blocked  = self._eval_blocked(agent)        # in [0, 1]  — higher is worse

        score = 0.5 * progress - 0.5 * blocked
