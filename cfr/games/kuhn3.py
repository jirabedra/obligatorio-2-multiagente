...
class KuhnPoker3(AlternatingGame):
    ...
    def random_change(self, agent: AgentID):
        ...
        other_players = [player for player in self.players if player != agent_idx]
        other_cards = self._cards.copy()
        other_cards.pop(agent_card)

        new_game = self.clone()
        new_game._hand[other_players] = np.random.choice(other_cards, size=len(other_players), replace=False)
        return new_game
