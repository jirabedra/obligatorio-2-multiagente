...
class Leduc(AlternatingGame):
    ...
    def __init__(self, render_mode='', seed=None):
        super().__init__()
    ...
    def reset(self, seed: int | None = None, options: dict | None = None) -> None:
        if seed is not None:
            ...
            self._current_seed = int(np.random.randint(0, 2**31 - 1))
        else:
            self._current_seed = self.seed
        self.env.reset(seed=self._current_seed, options=options)
        self._update()
    ...
    def clone(self):
        game = Leduc(render_mode=self.render_mode, seed=self._current_seed)
        game.reset(seed=self._current_seed)
